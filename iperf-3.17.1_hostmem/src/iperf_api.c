...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    //TCPX0
    while ((flag = getopt_long(argc, argv, "x:p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
        switch (flag) {
            //TCPX0
            case 'x':
            test->isTcpx = 1;
            test->ifname = optarg;
            test->ifindex = if_nametoindex(test->ifname);
            break;
            case 'p':
		portno = atoi(optarg);
