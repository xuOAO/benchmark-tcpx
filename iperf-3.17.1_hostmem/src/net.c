...
//TCPX0
const int ctrl_len = sizeof(int) * 200000;
static char ctrl_data[sizeof(int) * 200000];
static struct dmabuf_token token[256];
...
Nread(int fd, char *buf, size_t count, int prot)
{
    ...
    while (nleft > 0) {
        //TCPX0
        // XU_DEBUG("prot = %d, r = %d\n", prot, r);
        if(prot != 777)r = read(fd, buf, nleft);
        else r = tcpx_recv(fd, buf, nleft, 0, ctrl_data, ctrl_len);
            /*tcpx_recv_test(fd, buf, nleft, 0, ctrl_data, ctrl_len, token, maxLen);*/
        if (r < 0) {
