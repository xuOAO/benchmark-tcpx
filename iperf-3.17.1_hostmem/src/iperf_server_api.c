...
#define NUM_PAGES 16384
        int rule_idx = 0;
        SLIST_FOREACH(sp, &test->streams, streams) {
            int rport = ntohs(((struct sockaddr_in *) &sp->remote_addr)->sin_port);
            if(test->isTcpx == 1) {
                XU_DEBUG("ifindex = %d\n", test->ifindex);
                tcpx_preprocess(test->ifindex, 7, 1, 4096 * NUM_PAGES, NULL, HOST_MEM);
                open_nic_rfs(test->ifname, NULL, rport, NULL, 0, 7, rule_idx);
                // int flags = fcntl(sp->socket, F_GETFL, 0);
                // fcntl(sp->socket, F_SETFL, flags | O_NONBLOCK);
            } else {
                open_nic_rfs("eth0", NULL, rport, NULL, 0, 7, rule_idx);
                sp->isTcpx = 0;
            }
        }
                    if (test->protocol->id != Ptcp) {
